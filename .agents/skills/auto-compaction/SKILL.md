...
description: Turn this repository's Jev auto-compaction on or off for codex only, or show its status. Does not control built-in context-limit compaction.
---

This skill controls only codex. Never change another harness through it.

Run the bundled helper relative to this skill's directory:

```sh
python3 <skill-directory>/scripts/toggle.py off --target codex
```

Use `on`, `off`, or `status` as requested. Always use `--target codex`.
